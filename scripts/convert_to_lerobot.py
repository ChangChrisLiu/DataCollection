...
def main():
    ...
    for idx, ep_path in enumerate(episodes):
        ...
        # Phase filter
        frames = filter_frames_by_phase(frames, target_cfg["phases"])
        if not frames:
            skipped += 1
            continue
        ...
        convert_episode_frames(frames, dataset, args.task)
        dataset.save_episode()
        total_frames += len(frames)
        print(
            f"  Episode {idx}: {ep_path.name} -> {len(frames)} frames"
        )
    ...
    print(f"  Target:             {args.target}")
    print(f"  Repo ID:            {repo_id}")
    print(f"  Episodes converted: {len(episodes) - skipped}/{len(episodes)}")
